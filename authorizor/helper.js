...
module.exports.getJWKs = async (jwkPath) =>{
    var jwks =await axios.get('https://cognito-idp.eu-west-1.amazonaws.com/eu-west-1_LIN377rkm/.well-known/jwks.json')
        .then(({data})=>{
            return(data['keys']);
        })
...
module.exports.validateToken = (jwtToken, jwksPem,iss)=>{
    ...
    const res =jwt.verify(jwtToken,pem,{issuer:iss,maxAge:3600000},(err,payload)=>{
        if(err){
            switch(err.name){
